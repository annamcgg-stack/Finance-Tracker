...
import { Plus, Trash2, UserMinus, Mail, Check, Unlink, LogOut, History, Link2 } from "lucide-react";
...
export function HouseholdSettings() {
  ...
  const executeUnlink = async () => {
    ...
    try {
      ...
      if (unlinkAction.type === "leave") {
        ...
        await setDashboardView("personal");
        ...
        setMessage("Shared account unlinked.");
  ...
  const handleTransferAndLeave = async () => {
    ...
    try {
      ...
      await reloadHousehold();
      await loadLog();
      setMessage("Ownership transferred and you left the household.");
    } catch (e) {
  ...
  const handleAddSharedAccount = async () => {
    ...
    try {
      ...
      await reloadHousehold();
      await loadLog();
      setMessage("Shared account added.");
    } catch (e) {
  ...
  if (!household) {
    return (
      ...
      <Card className="space-y-6 p-5">
        <div>
          <h2 className="text-lg font-semibold text-foreground">Household</h2>
          <p className="mt-1 text-sm text-muted">
            Create or join a household to share selected finances with a partner or family.
          </p>
        </div>
        ...
        <div className="space-y-4 rounded-xl border border-border p-4">
          <h3 className="font-medium text-foreground">Create household</h3>
        ...
        <div className="space-y-4 rounded-xl border border-border p-4">
          <h3 className="flex items-center gap-2 font-medium text-foreground">
            <Link2 className="h-4 w-4" /> Join household
          </h3>
          ...
          <Button variant="secondary" onClick={handleJoin} disabled={!joinToken.trim()}>
            Join with invite
          </Button>
        </div>
      </Card>
  ...
  return (
    <div id="household" className="space-y-6">
      <Card className="space-y-2 p-5">
        <h2 className="text-lg font-semibold text-foreground">{household.name}</h2>
        <p className="text-sm text-muted">Manage members, shared accounts, and household linking.</p>
        {error && <p className="text-sm text-red-600">{error}</p>}
        {message && <p className="text-sm text-emerald-600">{message}</p>}
      </Card>
      ...
      <Card className="space-y-4 p-5">
        <h3 className="font-medium text-foreground">Leave household</h3>
            ...
            {!showTransferPanel ? (
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <Button onClick={handleTransferAndLeave} disabled={loading || !transferTargetId}>
                    Transfer & leave
                  </Button>
        ...
        {isOwner && ownerLeaveOptions?.requiresDelete && (
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              disabled={loading}
            >
              <Unlink className="h-4 w-4" /> Leave (delete empty household)
            </Button>
            <Button
              variant="danger"
              onClick={() => setUnlinkAction({ type: "delete_household" })}
